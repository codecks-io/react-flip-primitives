// taken from react-motion@0.5.2, added onAdd callback
...
export default function mergeDiff(prev, next, onRemove, onAdd) {
  ...
  for (let i = 0; i < next.length; i++) {
    ret[i] = next[i]
    if (!Object.prototype.hasOwnProperty.call(prevKeyIndex, next[i].key)) {
      onAdd(i, next[i])
    }
  }
  for (let i = 0; i < prev.length; i++) {
