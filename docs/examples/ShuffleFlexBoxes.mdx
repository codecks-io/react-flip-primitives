        ...
        <div style={{display: 'flex', flexWrap: 'wrap'}}>
          {keysAndData.map(({key, data: number}, index) => (
            <div
              ...
              ref={registerEnterLeave(key, {transitionProps: ["background-color", "opacity"], delayMs: index * 10})}
            />
          ))}
        </div>
      )}</EnterLeaveGroup>
      <button onClick={regenerate} ref={registerNode('button', {delayMs: numbers.length * 10})}>regenerate</button>
    </React.Fragment>
  )}</FlipGroup>
)}</GenerateNumbers>
