...
<GenerateNumbers min={10} max={50} count={10}>{(numbers, regenerate) => (
  <ReactFlip noAnimationOnMount changeKey={numbers.join('-')} durationMs={1500}>{registerFlip => (
    <React.Fragment>
      <LeaveEnter
      ...
      <button onClick={regenerate} ref={registerFlip('button')}>regenerate</button>
    </React.Fragment>
  )}</ReactFlip>
)}</GenerateNumbers>
